...
func fetchLatestVersion(ctx context.Context, token string) (string, error) {
	...
	if err := githubGet(ctx, token, "https://api.github.com/repos/caddyserver/caddy/releases/latest", &r); err != nil {
		return "", err
	}
...
func fetchTagCommit(ctx context.Context, token, tag string) (string, error) {
	...
	if err := githubGet(ctx, token, "https://api.github.com/repos/caddyserver/caddy/commits/"+tag, &c); err != nil {
		return "", err
	}
	return c.SHA, nil
}
